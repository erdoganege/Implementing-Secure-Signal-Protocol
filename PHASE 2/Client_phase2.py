...
h_msg, s_msg = generate_sign(P,n, Sa, m_bytes)
PseudoSendMsg(h_msg, s_msg)

requests = []
for i in range(5):
    request_message = ReqMsg(h_msg,s_msg)
...
for i in range(len(requests)):
    ...
    if calculated_hmac == hmac:
        ...
        Checker(m, client_ID, msgID, plaintext.decode("UTF-8"))
    else:
        print("Hmac couldn't be verified")
        Checker(m, client_ID, msgID, "INVALIDHMAC")
''''''
